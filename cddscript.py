...
final_structure = [] # now pair every run label with the sum of every date's cdd

for runlabel_list in new_structure:
    output_pair = [runlabel_list[0]]
    cdd65 = []
    for pair in runlabel_list[1]:
        compute_cdd65 = pair[1] - 65
        if compute_cdd65 < 0:
            compute_cdd65 = 0
        cdd65.append(compute_cdd65)
    output_pair.append(sum(cdd65))
...
data = {'runlabel':output_runlabels, 'CDDs':output_vals}
...
with pd.ExcelWriter(output_path) as writer:
    new_df.to_excel(writer, sheet_name="CDD Results", index=False)
